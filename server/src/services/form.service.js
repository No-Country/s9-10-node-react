...
export const createNewForm = async ({ title, questions, comments }) => {
  try {
    ...
    console.log(error);
    throw new HttpException("Error al crear el formulario", 500);
  }
};
  ...
  try {
    ...
    for (const ans of answers) {
      ...
      if (formQuestion.type === "closed") {
        ...
        if (typeof score !== "number" || score < scaleRange.min || score > scaleRange.max) {
          throw new HttpException(
    ...
    console.log(error);
    throw new HttpException("Error al crear la respuesta", 500);
  }
};